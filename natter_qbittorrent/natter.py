...
def closed_socket_ex(ex):
    ...
    if hasattr(errno, "WSAEINTR") and ex.errno == errno.WSAEINTR:
        return True
    if hasattr(errno, "WSAENOTSOCK") and ex.errno == errno.WSAENOTSOCK:
        return True
    return False
...
def update_qbittorrent_port(public_port, qb_url="http://127.0.0.1:18080", qb_username="admin", qb_password="adminadmin"):
    """Update qBittorrent listening port via Web UI API."""
    import urllib.request
    import urllib.parse
    
    Logger.info("Updating qBittorrent listening port to %d..." % public_port)
    
    try:
        # Login to get cookie
        login_url = "%s/api/v2/auth/login" % qb_url
        ...
        with urllib.request.urlopen(req, timeout=10) as response:
            ...
            if not cookie:
                Logger.error("qBittorrent: Failed to get authentication cookie")
                return False
        
        # Set preferences
        prefs_url = "%s/api/v2/app/setPreferences" % qb_url
        prefs_data = urllib.parse.urlencode({
            "json": '{"listen_port":"%d"}' % public_port
        }).encode()
        
        req = urllib.request.Request(prefs_url, data=prefs_data, method="POST")
        req.add_header("Referer", qb_url)
        req.add_header("Content-Type", "application/x-www-form-urlencoded")
        ...
        with urllib.request.urlopen(req, timeout=10) as response:
            if response.status != 200:
                Logger.error("qBittorrent: Failed to set preferences")
                return False
        
        # Logout
        logout_url = "%s/api/v2/auth/logout" % qb_url
        req = urllib.request.Request(logout_url, method="POST")
        req.add_header("Referer", qb_url)
        req.add_header("Cookie", cookie)
        ...
        Logger.info("qBittorrent: Successfully updated listening port to %d" % public_port)
        return True
        
    except Exception as ex:
        Logger.error("qBittorrent: Failed to update port: %s" % str(ex))
        return False
...
def natter_main(show_title = True):
    ...
    group.add_argument(
        "--qbittorrent", action="store_true", help="update qBittorrent listening port to public port"
    )
    group.add_argument(
        "--qb-url", type=str, metavar="<url>", default="http://127.0.0.1:18080",
        help="qBittorrent Web UI URL (default: http://127.0.0.1:18080)"
    )
    group.add_argument(
        "--qb-user", type=str, metavar="<username>", default="admin",
        help="qBittorrent Web UI username (default: admin)"
    )
    group.add_argument(
        "--qb-pass", type=str, metavar="<password>", default="adminadmin",
        help="qBittorrent Web UI password (default: adminadmin)"
    )
    ...
    qbittorrent_enabled = args.qbittorrent
    qb_url = args.qb_url
    ...
    if qbittorrent_enabled:
        outer_port = outer_addr[1]
        update_qbittorrent_port(outer_port, qb_url, qb_user, qb_pass)
